...
const url = process.env.NODE_ENV == "development" ? "http://localhost:3001" : "https://linksaver-server-lrtx.onrender.com"
...
class GetData {
	...
	async getPost(type, id) {
		if (this.posts.get(`${type}-${id}`)) {
			return this.posts.get(`${type}-${id}`)
		}
