...
export default function USERNAMEPAGE({ user }) {
	...
	const { data: folderData, loadMore: handleMoreFolders } = useLoadMore(`folders/?count=5&user_id=${user.id}`, {
		step: 5,
	})
	...
	return (
		<Box>
			...
			<SimpleGrid pb={8} minChildWidth={"200px"} justifySelf={"center"} gap={4}>
				{posts?.map((post, idx) => {
					if (post?.categories?.some((item) => selectedTags.includes(item)) || selectedTags.length == 0)
						return (
			...
			{posts.length < user?.post_count ? (
				<Flex justifyContent={"center"}>
					<Button w={"50%"} colorScheme={"pink"} shadow={"-3px 3px"} size={"lg"} rounded={"3xl"} onClick={loadMore}>
						Load More
					</Button>
				</Flex>
			) : null}
