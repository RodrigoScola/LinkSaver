...
describe('teste de rotas de folders', () => {
	it('pega todas as folders', async () => {
		const response = await App.get('/folders');
	...
	it('updates a folder', async () => {
		const original = (
			await dbconnection('folders').insert(
				{
					description: '',
					title: 'hello there',
					userId: -1,
		...
		const response = await App.put(`/folders/${original!.id}`).send({
			...original,
			title: 'hello there',
		});
