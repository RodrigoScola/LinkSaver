...
class QueryBuilderFactorySingleton {
    ...
    Remove<T extends object>(
        key: keyof ContextParameters<T>,
        item: ContextParameters<T>,
    ) {
        if (key in item) {
            delete item[key];
        }
    ...
    Search<T extends object>(
        item: object & { search_mode?: string | number },
        validItem: T,
        param: ContextParameters<T>,
        props?: QueryProps,
    ): this {
        if (
        ...
        if (
            ...
            !Formatter.isInEnum(
                Number(item.search_mode),
                SEARCH_MODE as unknown as Record<string, number>,
            )
        ) {
            return this;
        }
    ...
    Order<Valid extends object>(
        item: object,
        final: ContextParameters<Valid>,
    ): this {
        if (
        ...
        final.orders = [...item.orderBy].reduce((acc, key, i) => {
            ...
            if (
                ...
                item.order[i] &&
                (item.order[i] === 'asc' || item.order[i] === 'desc')
            ) {
    ...
    Make<T extends object>(
        item: object | undefined | null | qs.ParsedQs,
        tableItem?: T,
        props?: QueryProps,
    ): ContextParameters<T> {
        ...
        if (
            'offset' in item &&
            (typeof item.offset === 'string' || typeof item.offset === 'number')
        ) {
